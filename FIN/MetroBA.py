...
#Está función no la tenia Lu pero la dejo por si acaso
...
def mostrar_mapa(ruta):
    coords = []  # Lista para almacenar las coordenadas de la ruta
    
    # Recorremos la ruta y buscamos las coordenadas de cada estación
    for estacion in ruta:
        found = False
        index = 0
        while index < len(LISTA_COORDENADAS) and not found:
            linea, estaciones = list(LISTA_COORDENADAS.items())[index]  # Obtenemos las estaciones de cada línea
            if estacion in estaciones:
                coords.append(estaciones[estacion])  # Añadimos las coordenadas de la estación
                found = True
            index += 1
        
        # Si no encontramos las coordenadas de alguna estación, mostramos un error
        if not found:
            messagebox.showerror("Error", f"No se encontraron coordenadas para la estación: {estacion}.")
            return
    ...
    # Añadimos los marcadores de cada estación en la ruta
    for estacion, coord in zip(ruta, coords):
        folium.Marker(coord, popup=estacion).add_to(mapa)

    # Añadimos la línea azul que conecta las estaciones
    ...
    try:
        ...
        print("Mapa guardado correctamente como 'ruta_metro.html'.")  # Para depuración
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo guardar el mapa: {e}")
        return

    # Intentamos abrir el mapa en el navegador
    try:
        file_path = os.path.abspath("ruta_metro.html")  # Ruta absoluta para el archivo HTML
        webbrowser.open(f"file://{file_path}")
        print("Mapa abierto en el navegador.")  # Para depuración
    except Exception as e:
        messagebox.showerror("Error", f"No se pudo abrir el navegador: {e}")
        return


#ESTA CAMBIA UN POQUITO PERO HACE LO MISMO
def calcular_ruta():
    """
    ...
    """
    # Obtener las estaciones de origen y destino de los selectbox en Streamlit
    origen = st.session_state.estacion_origen  # Origen seleccionado
    destino = st.session_state.estacion_destino  # Destino seleccionado

    # Validación de entradas de origen y destino
    if not origen or not destino:
    ...
    # Obtener la hora y minuto de los selectbox en Streamlit
    hora = st.session_state.hora_var  # Hora seleccionada
    minuto = st.session_state.minuto_var  # Minuto seleccionado
