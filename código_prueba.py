...
LISTA_COORDENADAS = {
    ...
    "LINEA_E": {
        ...
        "Bolívar": [-34.60961123829135, -58.37401251606132] #TRANSBORDO 2
    }
}
...
def calcular_ruta():
    ...
    # Verificar si las estaciones de origen y destino son las mismas
    if estacion_inicio == estacion_destino_var:
        messagebox.showinfo("Ruta no necesaria", "Ya estás en la estación seleccionada.")
        return
    ...
    try:
        ...
        if es_festivo(fecha_viaje):
            messagebox.showinfo("Aviso", "Es un día festivo. Los horarios y servicios podrían verse modificados.")
        ...
        datetime_viaje = datetime.combine(fecha_viaje, hora_viaje)
        tiempo_total = timedelta(minutes=distancia_total / 500 * 60)
        hora_llegada = datetime_viaje + tiempo_total
...
ttk.Label(ventana, text="Hora de viaje:").grid(row=3, column=0, padx=10, pady=10)
